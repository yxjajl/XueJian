...
public class ActivityService {
	...
	public void loadPlayerActy(UserCached userCached) {// 登录时加载
		...
		if (playerActyVO == null) {// 兼容以前没有活动数据
			...
			sqlSaveThread.putSql(userCached.getPlayerId(), sqlBuild.getSql("com.dh.dao.PlayerActyMapper.insertPlayerActy", playerActyVO));
		} else {
			userCached.getUserActivity().setPlayerActyVO(playerActyVO);
		}
	}
	...
	public PlayerBossVO getPlayerBossVO(UserCached userCached) {
		...
		if (playerBossVO == null) {
			...
			userCached.getUserActivity().setPlayerBossVO(playerBossVO);
			sqlSaveThread.putSql(playerBossVO.getPlayerId(), sqlBuild.getSql("com.dh.dao.PlayerBossMapper.insertPlayerBoss", playerBossVO));
	...
	public void updatePlayerBoss(PlayerBossVO playerBossVO) {
		sqlSaveThread.putSql(playerBossVO.getPlayerId(), sqlBuild.getSql("com.dh.dao.PlayerBossMapper.updatePlayerBoss", playerBossVO));
	}

	public void updatePlayerActy(PlayerActyVO playerActyVO) {
		sqlSaveThread.putSql(playerActyVO.getPlayerid(), sqlBuild.getSql("com.dh.dao.PlayerActyMapper.updatePlayerActy", playerActyVO));
	}
	...
	public void loadBossMaxKill() {
		...
		if (playerBossVO1 == null) {
			...
			sqlSaveThread.putSql(0,sqlBuild.getSql("com.dh.dao.PlayerBossMapper.insertPlayerBoss", playerBossVO1));
		} else {
			BossLogVO bossLogVO = new BossLogVO();
		...
		if (playerBossVO2 == null) {
			...
			sqlSaveThread.putSql(0,sqlBuild.getSql("com.dh.dao.PlayerBossMapper.insertPlayerBoss", playerBossVO2));
		} else {
			BossLogVO bossLogVO = new BossLogVO();
	...
	public void actyAnsFresh(ActyAnsReq req) throws Exception {
		...
		if (req == null) {// 启动时调用
			isNeedOpen = Calendar.getInstance().get(Calendar.HOUR_OF_DAY) >= ActyTimer.ANS_START_TIMES[0] && Calendar.getInstance().get(Calendar.HOUR_OF_DAY) < ActyTimer.ANS_START_TIMES[1];
	...
	public void actyAnsEnd() throws Exception {
		...
		for (int i = 0; i < top3List.size(); i++) {
			...
			mailService.addNewMail(null, actyVO.getPlayerid(), mailTitle, content[0] + title[i] + content[1], MailConstants.MAIL_SENDER_ACTY_ANS, 0,
					MailService.createRewardList(null, RewardService.REWARD_TYPE_GOODS, rewardArray[i], 1));
