...
public class PlayerAccountService {
	...
	private void changeCurrency(PLAYER_PROPERTY type, int value, boolean isRecharge, PlayerAccountVO playerAccountVO, List<NettyMessageVO> commandList, String reason, int... gmio) throws Exception {
		...
		case PROPERTY_POWER:
			...
			if (value < 0) { // 消耗体力增加竞技值
				...
				sqlSaveThread.putSql(playerAccountVO.getPlayerId(),sqlBuild.getSql("com.dh.dao.PlayerAccountMapper.updatePlayerAccountPVP", playerAccountVO));
		...
		if (sql != null) {
			sqlSaveThread.putSql(playerAccountVO.getPlayerId(),sqlBuild.getSql(sql, playerAccountVO));
			RedisMap.updatePlayerAccount(playerAccountVO);
		} else {
			return;
		}
	...
	public void updatePowerTimer(PlayerTimerVO playerTimerVO) throws Exception {
		sqlSaveThread.putSql(playerTimerVO.getPlayerId(),sqlBuild.getSql("com.dh.dao.PlayerTimerMapper.updatePowerTimer", playerTimerVO));
	}

	public void updatePlayerPower(PlayerAccountVO playerAccountVO) throws Exception {
		sqlSaveThread.putSql(playerAccountVO.getPlayerId(),sqlBuild.getSql("com.dh.dao.PlayerAccountMapper.updatePlayerAccountPower", playerAccountVO));
	}
	...
	public void updatePlayerAccountRMB(PlayerAccountVO playerAccountVO) throws Exception {
		sqlSaveThread.putSql(playerAccountVO.getPlayerId(),sqlBuild.getSql("com.dh.dao.PlayerAccountMapper.updatePlayerAccountRMB", playerAccountVO));
	}
	...
	public void insertPlayerAccountLog(PlayerAccountVO playerAccountVO, int currency, int value, int beValue, String reason) throws Exception {
		...
		sqlSaveThread.putSql(0,sqlBuild.getSql("com.dh.dao.PlayerAccountMapper.insertPlayerAccountLog", playerAccountLog));
	}
