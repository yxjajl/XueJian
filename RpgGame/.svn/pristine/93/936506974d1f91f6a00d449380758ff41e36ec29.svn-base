...
public class RaidProcessor {
	...
	public void battleDetail(RaidPrepareRequest request, NettyMessageVO nettyMessageVO, List<NettyMessageVO> commandList) throws Exception {
		...
		IBattle battle = getIBattleInstance(request.getType());
	...
	public void raidUpdateHeroTeam(HeroTeam request, NettyMessageVO nettyMessageVO, List<NettyMessageVO> commandList) throws Exception {
		...
		if ((System.currentTimeMillis() - GameRecordConstants.LOPENSERVERTIME) <= DateUtil.WEEK_MILLIS) {
			...
			int index = OpenActRes.getInstance().getCombatIndex(combat);
			if (index > -1) {
				index++;
				if (WelfareProcessor.getWeiZi(playerWelfareVO.getDdhszlzw(), index, 0) == 0) {
					int temp = WelfareProcessor.setWeiZi(playerWelfareVO.getDdhszlzw(), index, 0);
		...
		if (playerArenaVO != null && playerArenaVO.getCombat() < combat) {
			...
			if (userCached.getUserHero().getAtkHeroList() != null && userCached.getUserHero().getAtkHeroList().size() > 0) {
				...
				arenaService.savePlayerHeroDefVO(playerId, userCached.getUserHero().getAtkHeroList());
	 ...
	 * @return
	 */
	public IBattle getIBattleInstance(BATTLE_TYPE type) {
		switch (type) {
		case RAID:
			return raidBattle;
		case ARENA:
