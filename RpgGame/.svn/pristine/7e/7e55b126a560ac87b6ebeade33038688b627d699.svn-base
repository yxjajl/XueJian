...
public class VipRewardRes extends BaseRes<Reward> {
	private static final Logger LOGGER = Logger.getLogger(VipRewardRes.class);
	public static final String Path = filePath + "csv/cfg_vipreward.csv";

	public Map<Integer, List<Reward>> map = new HashMap<Integer, List<Reward>>();

	private VipRewardRes() {
		classz = Reward.class;
	}

	private static VipRewardRes INSTANCE = new VipRewardRes();

	public static VipRewardRes getInstance() {
		return INSTANCE;
	}

	public void otherInit() {
		LOGGER.info("VipRewardRes.otherInit");
	...
	public static void main(String[] args) {
		VipRewardRes.getInstance().loadFile(VipRewardRes.Path);
		ItemRes.getInstance().loadFile(ItemRes.Path);

		// for (Reward reward : VipRewardRes.getInstance().getDataList()) {
		...
		// }
		//
		for (Reward reward : VipRewardRes.getInstance().getRewardRateGroup(1)) {
			System.out.println(reward.getId() + "," + reward.getContent() + "," + reward.getNumber());
		}
