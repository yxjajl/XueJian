...
public class PlayerService {
	...
	public void loadAllPlayer(List<PlayerVO> list) {
		...
		for (PlayerVO playerVO : list) {
			RoleLevelRank.getInstance().putRoleLevel(playerVO.getPlayerId(), playerVO.getLevel());
		}
	}
	...
	public void addPlayerExp(UserCached userCached, int exp, List<NettyMessageVO> commandList) throws Exception {
		...
		try {
			...
			RoleLevelRank.getInstance().putRoleLevel(userCached.getPlayerVO().getPlayerId(), userCached.getPlayerVO().getLevel());
		} catch (Exception e) {
			logger.error("RoleLevelRank.getInstance().putRoleLevel error ", e);
		}
	}
